...
const AdminPanel = () => {
  ...
  const requestEmailCode = async () => {
    try {
      setSending(true);
      const apiUrl = process.env.NODE_ENV === 'production' ? '/api/auth/request-code' : 'http://localhost:3001/api/auth/request-code';
      const res = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: ADMIN_EMAIL }) });
  ...
  const verifyEmailCode = async () => {
    ...
    try {
      const apiUrl = process.env.NODE_ENV === 'production' ? '/api/auth/verify-code' : 'http://localhost:3001/api/auth/verify-code';
      const res = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: ADMIN_EMAIL, code: emailCode }) });
      const data = await res.json();
      if (data.success) {
